package ficheroaccesoaleatorio_2;
...
El metodo más interesante es insertar(). Tiene dos variantes. Si no se le indica la posición,
...
public class FicheroAccesoAleatorio_2 {
     ...
     */
    FicheroAccesoAleatorio_2(String nomFich, List<Pair<String, Integer>> campos)
            throws IOException {
        this.campos = campos;
    ...
    /**
     * Devuelve el valor de un campo de un registro dado.
     * @param pos La posición del registro (0 para el primero).
     * @param nomCampo El nombre del campo cuyo valor se desea obtener.
     * @return El valor del campo como String.
     * @throws IOException Si ocurre un error de E/S.
     */
    public String obtenValorCampo(long pos, String nomCampo) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(f, "r")) {
            // Calcula la posición inicial del registro
            raf.seek(pos * longReg);

            // Recorre los campos hasta encontrar el deseado
            for (Pair<String, Integer> campo : campos) {
                String nombre = campo.getKey();
                int longitud = campo.getValue();
                byte[] bytesCampo = new byte[longitud];

                // Lee los bytes del campo actual
                raf.readFully(bytesCampo);

                if (nombre.equals(nomCampo)) {
                    // Convierte a String y devuelve
                    return new String(bytesCampo, "UTF-8").trim();
                }
            }
        }

        // Si no se encuentra el campo, se puede lanzar una excepción
        throw new IllegalArgumentException("Campo " + nomCampo + " no existe.");
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            FicheroAccesoAleatorio_2 faa = new FicheroAccesoAleatorio_2("src/ficheroaccesoaleatorio_2/fic_acceso_aleat_2.dat", campos);
            ...
            System.out.println("Nombre del registro 1: " + faa.obtenValorCampo(1, "NOMBRE"));
            System.out.println("DNI del registro 0: " + faa.obtenValorCampo(0, "DNI"));
